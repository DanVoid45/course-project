...
"""Нужно доработать, чтобы сначала показывалось сообщение в приложении, а потом уже все говорилось 
Да и в принципе сделать, чтобы все обрабатывалось быстрее"""
...
def callback_ui_threadsafe(text, is_voice=False):
    # Просто положить результат в очередь
    voice_queue.put((text, is_voice))

def process_queue():
    # Проверяем очередь в главном потоке
    try:
        while True:
            text, is_voice = voice_queue.get_nowait()
            print(">>> Получено из очереди:", text)
...
def click_handler_voice():
    print(">>> Старт голосового ввода")
    functions.listen(callback_ui=callback_ui_threadsafe)


def click_handler_text(request, is_voice=False):
    if not request or request.strip() == "" or request == "Голос не распознан!" or request == "Ошибка соединения!":
        return
    
    if not is_voice: entry.delete(0, END)
    ...
    response_text = functions.execute_cmd(request, cmd = cmd_result)
